...
namespace gcgcg
{
  public class Mundo : GameWindow
  {
    ...
    protected override void OnLoad()
    {
      ...
      circuloMaior = new Circulo(mundo, ref rotuloAtual, new Ponto4D(0, 0.0), 0.7)
      {
          PrimitivaTipo = PrimitiveType.LineLoop
      };

      circuloMenor = new Circulo(mundo, ref rotuloAtual, new Ponto4D(0, 0.0), 0.25)
      {
          PrimitivaTipo = PrimitiveType.LineLoop
      };
      #endregion
    ...
    protected override void OnUpdateFrame(FrameEventArgs e)
    {
        ...
        if (estadoTeclado.IsKeyDown(Keys.E)) // Esquerda
        {
            movimentoX -= delta;
        }
        if (estadoTeclado.IsKeyDown(Keys.C)) // Cima
        {
            movimentoY += delta;
        }
        if (estadoTeclado.IsKeyDown(Keys.B)) // Baixo
        {
            movimentoY -= delta;
        }
