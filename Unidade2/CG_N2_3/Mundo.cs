...
namespace gcgcg
{
  public class Mundo : GameWindow
  {
    ...
    protected override void OnLoad()
    {
      ...
      objetoSelecionado = new SrPalito(mundo, ref rotuloAtual, new Ponto4D(-0.25, -0.25), new Ponto4D(0.25, 0.25));
      #endregion
    }
    ...
    protected override void OnUpdateFrame(FrameEventArgs e)
    {
      ...
      if (estadoTeclado.IsKeyDown(Keys.Q))
          (objetoSelecionado as SrPalito)?.Mover(-0.001f);  // Mover para esquerda
      if (estadoTeclado.IsKeyDown(Keys.W))
          (objetoSelecionado as SrPalito)?.Mover(0.001f);   // Mover para direita

      // Ajuste de tamanho
      if (estadoTeclado.IsKeyDown(Keys.A))
          (objetoSelecionado as SrPalito)?.AlterarTamanho(0.995f);  // Diminuir tamanho
      if (estadoTeclado.IsKeyDown(Keys.S))
          (objetoSelecionado as SrPalito)?.AlterarTamanho(1.005f);  // Aumentar tamanho

      // Rotação
      if (estadoTeclado.IsKeyDown(Keys.Z))
          (objetoSelecionado as SrPalito)?.Girar(-0.0001f);  // Girar sentido anti-horário
      if (estadoTeclado.IsKeyDown(Keys.X))
          (objetoSelecionado as SrPalito)?.Girar(0.0001f);   // Girar sentido horário
